...
import sys
import uproot
from sificc_lib import Event_new, SiFiCC_Module
from tqdm import tqdm

class Simulation_new:
    '''Process a ROOT simulation for SiFi-CC detection'''
    def __init__(self, file_name, clusters_limit=8):
    ...
    def iterate_events(self, basket_size=100000, desc='processing root file', 
        ...
        bar_step = 0
        for start, end, basket in self.tree.iterate(Event_new.l_leaves, entrysteps=basket_size, 
    ...
    def get_event(self, position):
        ...
        '''
        for basket in self.tree.iterate(Event_new.l_leaves, entrystart=position, entrystop=position+1, 
                                        namedecode='utf-8'):
    ...
    def __event_at_basket(self, basket, position):
        ...
        event = Event_new(simulated_event_type = basket['SimulatedEventType'][position],
                      real_primary_energy = basket['Energy_Primary'][position], 
