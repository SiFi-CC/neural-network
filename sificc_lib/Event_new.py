...
import math
import numpy as np
from sificc_lib import Utils_new
...
class Event_new:
    ...
    l_leaves = ['SimulatedEventType', 'Energy_Primary', 'RealEnergy_e', 'RealEnergy_p', 'RealPosition_source', 
    ...
    def __init__(self, simulated_event_type, real_primary_energy, real_e_energy, real_p_energy, real_e_positions, 
        ...
        self.simulated_event_type = simulated_event_type 
        ...
        if self.is_distributed_clusters \
                and self.simulated_event_type == 2:
            self.is_distributed_clusters_realcoinc = True
            self.is_distributed_clusters_randcoinc = False
        elif self.is_distributed_clusters \
                and self.simulated_event_type == 3:
            self.is_distributed_clusters_randcoinc = True
            self.is_distributed_clusters_realcoinc = False
        else:
            self.is_distributed_clusters_realcoinc = False
            self.is_distributed_clusters_randcoinc = False
    ...
    def _count_matching_clusters(self, point):
        ...
        for cluster, cluster_unc in zip(self.clusters_position, self.clusters_position_unc):
            if Utils_new.is_point_inside_cluster(point, cluster, cluster_unc):
                count += 1
        return count
    ...
    def _arg_closest_cluster(self, point):
        ...
        closest_idx = 0
        min_euclidean = Utils_new.euclidean_distance(point, self.clusters_position[0])
        
        for idx, cluster in enumerate(self.clusters_position):
            distance = Utils_new.euclidean_distance(point, cluster)
    ...
    def get_features(self):
        ...
        for i in range(self.clusters_limit):
            cluster = np.concatenate((
                ...
                Utils_new.vec_as_np(self.clusters_position[i]),
                np.abs(Utils_new.vec_as_np(self.clusters_position_unc[i])),
            ))
    ...
    def get_targets(self):
        ...
        if self.is_ideal_compton:
            ...
            targets = np.concatenate((
                ...
                [self.real_e_energy],
                [self.real_p_energy],
                Utils_new.vec_as_np(self.real_e_position),
